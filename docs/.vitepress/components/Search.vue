...
const handleSearch = (event: any) => {
  ...
  isSearchStrictMode.value =
    searchTerm.charAt(0) === '"' &&
    searchTerm.charAt(searchTerm.length - 1) === '"';
  searchTerm = searchTerm
    .toLowerCase()
    .replace(regexForContentStripping.value, '');
  ...
  const newResults = filesToSearch.filter((file) => {
    if (file.title) {
      const doesInclude = containsSearchTerm(
        searchTerms,
  ...
  searchResults.value = newResults.map((data) => {
    ...
    return {
      ...data,
      content: '...' + data.content.slice(index, data.content.length),
    };
  });
  msToSearch.value = Date.now() - startTime;
};
...
onMounted(async () => {
  const importData = await import('vitepress-plugin-simple-search');
  try {
    fileData.value = importData.default.data;
    searchResults.value = importData.default.data;
    regexForContentStripping.value =
      importData.default.regexForContentStripping;
  } catch (err) {
